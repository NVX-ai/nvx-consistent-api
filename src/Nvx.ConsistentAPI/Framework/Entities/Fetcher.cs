  ...
  where Entity : EventModelEntity<Entity>
{
    ...
    InterestFetcher interestFetcher)
  {
    ...
    async Task<FetchResult<Entity>> Fetch(Entity defaulted, Position? upToRevision, Fetcher fetcher, bool resetCache)
    {
      ...
      return interests.Length == 0
        ? await FromSingle(cache, resetCache, defaulted, upToRevision, client, parser, entryOptions, fetcher)
        : await FromConcerned();

      async Task<FetchResult<Entity>> FromConcerned()
      {
  ...
  private static async Task<FetchResult<Entity>> FromSingle(
    MemoryCache cache,
    bool resetCache,
    Entity defaulted,
    Position? upToRevision,
